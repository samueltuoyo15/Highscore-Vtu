...
const transactions = [
    ...
    { id: 2, title: "Wallet Funding", desc: "Paystack", amount: "+₦5,000.00", date: "Yesterday, 4:15 PM", status: "Success", color: "text-emerald-500" },
            ...
            <div className="bg-white p-4 sticky top-0 z-10 shadow-sm border-b">
                <div className="flex items-center gap-4 max-w-2xl mx-auto">
                    ...
                    <div className="ml-auto flex gap-2">
                        <button className="p-2 text-gray-500 hover:text-primary"><Search className="w-5 h-5" /></button>
                        <button className="p-2 text-gray-500 hover:text-primary"><Filter className="w-5 h-5" /></button>
                    </div>
                </div>
            </div>
                ...
                {transactions.map((tx) => (
                    <div key={tx.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center hover:shadow-md transition-shadow">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.status === 'Success' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                        ...
                        <div className="text-right">
                            ...
                            <div className={`text-xs font-medium ${tx.status === 'Success' ? 'text-emerald-600' : 'text-red-600'}`}>{tx.status}</div>
                        </div>
                    </div>
                ))}
